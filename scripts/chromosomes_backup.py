...
# Path to the GTF file and gene list
gtf_file = "~/tesi/Dati/t2t/hs1.ncbiRefSeq.gtf.gz"
chr_list = "~/tesi/Dati/t2t/gene_list"
...
def process_chromosome(chromosome):
    ...
    output_dir = os.path.expanduser(f"~/tesi/Dati/t2t/annotations/chr_{chromosome}")
    ...
    # Build the command
    cmd = f"""gunzip -c {gtf_file} | awk '$1 == "chr{chromosome}" && $3 == "exon"' | sort -k4,4V"""
