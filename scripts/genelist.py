...
def process_chromosome(chromosome):
    ...
    output_dir = os.path.expanduser(f"~/tesi/Dati/t2t/chromosome_{chromosome}")
    ...
    # Build the command
    cmd = f"""gunzip -c {gtf_file} | awk '$1 == "chr{chromosome}" && $3 == "transcript"' | cut -f 9 | sed 's/; /\t/g; s/;$//' | cut -f 3 | sed 's/gene_name "\(.*\)"/\\1/' | uniq """
